...
const PathPrefix = 'shugiin-data'

export class Path {
	public static toRaw回次Path = (回次: string, prefix = PathPrefix) => `${prefix}/raw/${回次.padStart(4, '0')}`

	public static toRaw国会JsonName = () => '国会.json'

	public static toRaw国会PathBy回次 = (k: string, prefix = PathPrefix) => `${Path.toRaw回次Path(k, prefix)}/${Path.toRaw国会JsonName()}`

	public static toRaw国会Path = (k: Raw国会, prefix = PathPrefix) => `${Path.toRaw国会PathBy回次(k.国会回次, prefix)}`

	public static toRaw議案JsonlName = () => '議案.jsonl'

	public static toRaw議案PathBy回次 = (k: string, prefix = PathPrefix) => `${Path.toRaw回次Path(k, prefix)}/${Path.toRaw議案JsonlName()}`

	public static toRaw議案Path = (g: Raw議案, prefix = PathPrefix) => `${Path.toRaw議案PathBy回次(g.議案審議回次, prefix)}`

	public static toRaw経過JsonName = (k: Raw議案 | Raw経過) => '経過'
		+ '-' + path.normalize(toRaw議案Id(k))
		+ '.json'

	public static toRaw経過Path = (k: Raw議案 | Raw経過, prefix = PathPrefix) => `${Path.toRaw回次Path(k.議案審議回次, prefix)}/${Path.toRaw経過JsonName(k)}`

	public static to議案Path = (g: 議案, prefix = PathPrefix) => `${prefix}/gian/${g.議案Id}.json`

	public static to衆議院議案DataPath = (d: 衆議院議案Data, prefix = PathPrefix) => `${prefix}/衆議院議案Data.json`
}
...
export class Writer {
	...
	public static write衆議院議案Data(d: 衆議院議案Data) {
		fs.writeFileSync(Path.to衆議院議案DataPath(d), JSON.stringify(d, null, 2))
	}
}
